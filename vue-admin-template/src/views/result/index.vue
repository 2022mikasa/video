<template>
  <div class="video-list">
    <el-row>
      <el-col :span="6">
        <el-input v-model="keyword" placeholder="请输入搜索关键词" prefix-icon="el-icon-search" />
      </el-col>
    </el-row>
...
export default {
  ...
  methods: {
    ...
    fetchVideos() {
      axios.get('http://localhost:5000/result/list', {
        params: {
    ...
    deleteVideo(id) {
      if (window.confirm('您确定要删除这个视频吗？')) {
        axios.get('http://localhost:5000/result/delete?id=' + id).then(response => {
